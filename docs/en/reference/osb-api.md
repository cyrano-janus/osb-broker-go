...
| `PATCH /v2/service_instances/:id` | Update. Checks `allowedParameters`, re-renders, writes only on a real change; `200`. Switching to a different plan is **422** `PlanChangeNotSupported` unless the plan the instance is on promises it |
